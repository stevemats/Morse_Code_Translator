...
root = tkinter.Tk()
root.title('Morse Code Encoder and Decoder')
root.iconbitmap('images/icons/morse.ico')
...
def get_morse():
    ...
    # Turn each individual word in word list into a list of letters

    for word in word_list:
        letters = list(word)

        # For each letter, get the morse code representation and add it to the string morse_code
...
def clear():
    ...
    # For text widget, starting index is "1.0"
...
def show_guide():
    ...
    # Image 'morse' needs to be a global variable to put on a second window
    # Window 'guide' needs to be global to close in other function
    ...
    # Create a close button and pack

    close_button = tkinter.Button(guide, text='Close',
...
morse_to_english = dict([(value, key) for (key, value) in
                        english_to_morse.items()])
...
input_text = tkinter.Text(input_frame, height=8, width=30,
                          bg=text_color)
...
guide_button = tkinter.Button(input_frame, text='Guide',
                              font=button_font, bg=button_color,
                              command=show_guide)
...
output_text = tkinter.Text(output_frame, height=8, width=30,
                           bg=text_color)
output_text.grid(row=0, column=1, rowspan=4, padx=5, pady=5)

convert_button = tkinter.Button(output_frame, text='Convert',
                                font=button_font, bg=button_color,
                                command=convert)
play_button = tkinter.Button(output_frame, text='Play Morse',
                             font=button_font, bg=button_color,
                             command=play)
clear_button = tkinter.Button(output_frame, text='Clear',
                              font=button_font, bg=button_color,
                              command=clear)
quit_button = tkinter.Button(output_frame, text='Quit',
                             font=button_font, bg=button_color,
